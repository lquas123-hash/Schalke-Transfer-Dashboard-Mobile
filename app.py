...
# Mobiles CSS für echten App-Charakter auf dem Handy
st.markdown("""
    <style>
...
def load_data(sheet_id):
    ...
    # Wir lesen sauber ein, ohne Zeilen blind zu löschen
    df = pd.read_csv(csv_url, header=2)
    df = df.fillna("") # Verhindert, dass unvollständige Zeilen gelöscht werden
...
try:
    ...
    st.title("⚽ Schalke Transfer-Dashboard")
    st.caption("Live-Transfers & Statistiken")
    st.divider()

    # 1. Filter-Bereich (Standardmäßig zugeklappt, damit das Handy nicht zugemüllt wird)
    with st.expander("🔍 Filter anpassen (Saison, Manager, Position, Alter, Preis...)", expanded=False):
    ...
    # 2. Kennzahlen-Übersicht (Direkt im Blickfeld)
    with st.expander("📊 Kennzahlen-Übersicht", expanded=True):
        if not df_filtered.empty:
            col1, col2, col3, col4, col5 = st.columns(5)
            col1.metric("Transfers", len(df_filtered))
            sum_spend = df_filtered['Ablöse_numerisch'].sum() / 1e6
            col2.metric("Ausgaben", f"{sum_spend:.2f} Mio")
            ...
            col4.metric("Ø Alter", f"{avg_age:.1f} J" if pd.notna(avg_age) else "—")
            ...
            if not valid_fees.empty:
                ...
                top_str = f"{max_row['Spieler']} ({top_val:.1f}M)"
            else:
                top_str = "—"
            col5.metric("Top-Transfer", top_str)
    ...
    # 4. Standard Analysen & Grafiken
    st.subheader("📈 Analysen & Grafiken")
    ...
    if grafik_auswahl != "Keine":
        ...
        if col in df_raw.columns:
            ...
            fig.update_layout(xaxis={"type": "category", "tickangle": -45, "fixedrange": True}, yaxis={"fixedrange": True}, height=420, margin=dict(l=10, r=10, t=30, b=80))
            st.plotly_chart(fig, use_container_width=True, config=MOBILE_PLOTLY_CONFIG)

    st.divider()

    # 5. Manager-Vergleich Tabs
    with st.expander("📊 Manager-Vergleich", expanded=False):
        tab1, tab2, tab3, tab4 = st.tabs(["🎯 Alter vs. Preis", "💰 Ausgaben", "🎂 Alter", "⚽ Positionen"])
        age_colors = {"< 21 Jahre": "#2ca02c", "21–25 Jahre": "#1f77b4", "26–29 Jahre": "#ff7f0e", "30+ Jahre": "#d62728"}
        ...
        with tab1:
            ...
            if not df_scatter.empty:
                ...
                fig_scatter = px.scatter(df_scatter, x="Alter_numerisch", y="Ablöse_Mio", hover_data=["Spieler", "Manager", "Position", "Saison"], color_discrete_sequence=["#2b7fff"])
                fig_scatter.update_layout(xaxis={"fixedrange": True}, yaxis={"fixedrange": True}, height=420, margin=dict(l=10, r=10, t=20, b=40))
        ...
        with tab2:
            ...
            fig_vol.update_layout(xaxis={"type": "category", "tickangle": -45, "fixedrange": True}, yaxis={"fixedrange": True}, height=420, margin=dict(l=10, r=10, t=40, b=80))
        ...
        with tab3:
            ...
            if not df_age.empty:
                df_age["Altersgruppe"] = pd.cut(df_age["Alter_numerisch"], bins=[0, 20, 25, 29, 100], labels=["< 21 Jahre", "21–25 Jahre", "26–29 Jahre", "30+ Jahre"])
                fig_age = px.histogram(df_age, x="Manager", color="Altersgruppe", barmode="stack", color_discrete_map=age_colors)
                fig_age.update_layout(xaxis={"type": "category", "tickangle": -45, "fixedrange": True}, yaxis={"fixedrange": True}, height=450, margin=dict(l=10, r=10, t=20, b=100))
        ...
        with tab4:
            if "Position" in df_raw.columns:
                ...
                fig_pos.update_layout(xaxis={"type": "category", "tickangle": -45, "fixedrange": True}, yaxis={"fixedrange": True}, height=480, margin=dict(l=10, r=10, t=20, b=120))
    ...
    with st.expander("🛠️ Custom Chart Builder", expanded=False):
        ...
        x_col = c1.selectbox("X-Achse:", available_x, index=0)
        y_metric = c2.selectbox("Metrik:", ["Gesamtausgaben (Mio. €)", "Anzahl Transfers", "Ø Ablöse (Mio. €)", "Ø Alter"], index=0)
        chart_type = c3.selectbox("Typ:", ["Balkendiagramm", "Kreisdiagramm"], index=0)
        ...
        if not df_custom.empty:
            ...
            fig_c.update_layout(xaxis={"type": "category", "tickangle": -45, "fixedrange": True}, yaxis={"fixedrange": True}, height=450, margin=dict(l=10, r=10, t=40, b=80))
